...
import kotlinx.android.synthetic.main.item_editors_target.*

/**
 * Created by user on 3/26/18.
 */
class EditorsViewHolder(val context: Context,
    ...
    override fun onBind(target: GiftTarget) {
        ...
        label_editor_target.text = target.label
    }
    ...
    class Factory(val listener: (GiftTarget)->Unit): ViewHolderFactory<GiftTarget>() {
        override fun createViewHolder(parent: ViewGroup): BaseViewHolder<GiftTarget> {
            ...
            val containerView: View = inflater.inflate(R.layout.item_editors_target, parent, false)
            return EditorsViewHolder(context, containerView, listener)
        }
    }
}